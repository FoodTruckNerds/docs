...
. Navigate to the `site` folder in the `docs` repository:
+
[source,bash]
----
cd docs/site
...
content:
  sources:
    - url: https://github.com/FoodTruckNerds/common.git
      branches: HEAD
...
. **Local Development**: The `antora-local-playbook.yml` uses local paths for faster iteration and testing of documentation changes without needing to commit and push to GitHub.

. **Repository Structure**: Some local repositories may have different remote URLs than what's specified in the GitHub playbook (e.g., `common` repository might point to `workspace-settings.git`). The local playbook works around this by using filesystem paths.
